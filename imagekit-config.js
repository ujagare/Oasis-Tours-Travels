...
if (typeof ImageKitOptimizer === "undefined") {
  ...
  function optimizeExistingImages() {
    console.log(
      "ImageKit optimization is temporarily disabled to prevent image loading issues"
    );
    return;
    ...
    images.forEach((img) => {
      ...
      const imagePath =
        originalSrc.split("/Images/")[1] || originalSrc.split("/images/")[1];
      ...
      if (imagePath) {
        ...
        const optimizedUrl = imageKit.getOptimizedImageUrl(
          `Images/${imagePath}`,
          {
            width: width,
            quality: 85,
  ...
  function optimizeExistingVideos() {
    console.log(
      "ImageKit video optimization is temporarily disabled to prevent video loading issues"
    );
    return;
    // Only optimize videos that are specifically marked for optimization
    const videos = document.querySelectorAll(
      'video[data-optimize="true"] source[src*="video/"], video[data-optimize="true"][src*="video/"]'
    );
    ...
    videos.forEach((video) => {
      ...
      if (videoPath) {
        const optimizedUrl = imageKit.getOptimizedVideoUrl(
          `video/${videoPath}`,
          {
            quality: 70,
            format: "mp4",
